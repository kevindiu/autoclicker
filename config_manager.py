import os
import json
import time
import state
from theme import CONFIG_EXT

# ==============================================================================
# 設定檔管理 (Config Manager)
# 負責 .shm 設定檔之檔案列表讀取、資料序列化儲存、載入與 Schema 向後相容補齊
# ==============================================================================

def get_profile_files(ext=CONFIG_EXT, dir_path=".") -> list:
    """獲取指定目錄下所有設定檔名稱列表 (已去除副檔名並排序)"""
    try:
        return sorted([f[:-len(ext)] for f in os.listdir(dir_path) if f.endswith(ext)])
    except OSError:
        return []

def ensure_default_profile(ext=CONFIG_EXT, dir_path="."):
    """若無任何設定檔則建立預設 default 設定檔"""
    fn = os.path.join(dir_path, f"default{ext}")
    if not os.path.exists(fn):
        try:
            with open(fn, "w", encoding="utf-8") as f:
                json.dump({"variables": {}, "combos": [], "steps": [], "periodic_tasks": []}, f, ensure_ascii=False, indent=2)
        except OSError:
            pass

def save_profile_file(name: str, app_state=None, ext=CONFIG_EXT, dir_path=".") -> str:
    """將狀態資料儲存為 .shm 設定檔"""
    target_state = app_state if app_state is not None else state
    fn = os.path.join(dir_path, f"{name}{ext}")
    data = {
        "variables": target_state.variables,
        "combos": target_state.combos,
        "steps": target_state.steps,
        "periodic_tasks": target_state.periodic_tasks
    }
    with open(fn, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    return fn

def load_profile_file(name: str, app_state=None, ext=CONFIG_EXT, dir_path=".") -> dict:
    """從 .shm 設定檔載入設定資料，並自動補齊缺失之欄位與 ID"""
    target_state = app_state if app_state is not None else state
    fn = os.path.join(dir_path, f"{name}{ext}")
    if not os.path.exists(fn):
        raise FileNotFoundError(f"找不到檔案：{fn}")

    with open(fn, "r", encoding="utf-8") as f:
        data = json.load(f)

    target_state.variables.clear()
    target_state.variables.update(data.get("variables", {}))

    target_state.combos.clear()
    target_state.combos.extend(data.get("combos", []))

    target_state.steps.clear()
    target_state.steps.extend(data.get("steps", []))

    target_state.periodic_tasks.clear()
    for p_idx, pt in enumerate(data.get("periodic_tasks", [])):
        if not pt.get("id"):
            pt["id"] = f"pt_{int(time.time()*1000)}_{p_idx}"
        target_state.periodic_tasks.append(pt)

    return data

def get_data_snapshot(app_state=None) -> str:
    """獲取當前設定資料的序列化字串，用於精確對比是否有未儲存的變更"""
    target_state = app_state if app_state is not None else state
    try:
        return json.dumps({
            "variables": target_state.variables,
            "combos": target_state.combos,
            "steps": target_state.steps,
            "periodic_tasks": target_state.periodic_tasks
        }, sort_keys=True)
    except (TypeError, ValueError):
        return ""

def has_unsaved_changes(last_saved_snapshot: str, app_state=None) -> bool:
    """檢查當前記憶體中的設定相較於最後儲存狀態是否有更新"""
    if not last_saved_snapshot:
        return False
    return get_data_snapshot(app_state) != last_saved_snapshot
